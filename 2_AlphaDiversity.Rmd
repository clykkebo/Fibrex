---
title: "Fibrex Microbiome Alpha Diversity"
author: "masmo, caly"
date: "`r Sys.Date()`"
output:
...
Keeping in mind possible batch effects, now we can test for project effects. Depending on the project it might be best to test each variable individually or to perform a nested test of the variables. It is important to ensure that R has interpreted the selected variable as a factor, character variables will automatically be interpreted as factors, but variables that consists solely of numbers (numerical or integers) is interpreted as continous by R and must be transformed to be factors.
Individual factors to be tested in Fibrex are: feed, material, treatment, day, group -> feed_treat, feed_treat_day
Nested factors: 

## FEED (Day 0 only feces)

Here is code to test alpha diversity for the feed categorical variable.
...
# Subset
adat <- adat[adat$material == "Feces" & adat$day == 0,]
...
# If there is a significant batch effect, then it will be necessary to correct following tests for this effect.
filename <- paste0("plots/adiv_d0_feces_",TEST.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sdi, p.fpd, nrow = 1, labels = c("A)","B)","C)"), common.legend = TRUE,legend = "bottom")
(adiv_plot_title <- annotate_figure(adiv_plot, top = text_grob("Difference in alpha diversity between feed at day 0:", color = "red", face = "bold", size = 8)))
adiv_plot_title

suppressMessages(ggsave(filename = filename, plot = adiv_plot_title, device = "png"))
...
## TREATMENT (Day 0 only feces)

Here is code to test alpha diversity for the treatment categorical variable.
...
filename <- paste0("plots/adiv_d0_feces_",TEST.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sdi, p.fpd, nrow = 1, labels = c("A)","B)","C)"), common.legend = TRUE,legend = "bottom")
(adiv_plot_title <- annotate_figure(adiv_plot, top = text_grob("Difference in alpha diversity between treatment at day 0:", color = "red", face = "bold", size = 8)))
...
## FEED AND TREAT (Day 0 only feces)
...
adiv_plot <- ggarrange(p.obs,p.sha, nrow = 1, labels = c("A)","B)"), common.legend = TRUE,legend = "bottom",align = "hv")
...
# Subset
adat <- adat[adat$material == "Feces" & adat$day == 8,]
...
# If there is a significant batch effect, then it will be necessary to correct following tests for this effect.
filename <- paste0("plots/adiv_d8_feces_",TEST.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sdi, p.fpd, nrow = 1, labels = c("A)","B)","C)"), common.legend = TRUE,legend = "bottom")
(adiv_plot_title <- annotate_figure(adiv_plot, top = text_grob("Difference in alpha diversity between feed at day 8:", color = "red", face = "bold", size = 8)))
adiv_plot_title
...
filename <- paste0("plots/adiv_d8_feces_",TEST.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sdi, p.fpd, nrow = 1, labels = c("A)","B)","C)"), common.legend = TRUE,legend = "bottom")
(adiv_plot_title <- annotate_figure(adiv_plot, top = text_grob("Difference in alpha diversity between treatment at day 8:", color = "red", face = "bold", size = 8)))
...
## FEED AND TREAT (Day 8 only feces)
...
### Create output plot
filename <- paste0("plots/adiv_d8_",OUTER.VAR,"_",INNER.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sha, nrow = 1, labels = c("A)","B)"), common.legend = TRUE,legend = "bottom",align = "hv")
suppressMessages(ggsave(filename = filename, plot = adiv_plot, device = "png"))
...
# Subset
adat <- adat[adat$material == "Feces" & adat$day == 21,]
...
# If there is a significant batch effect, then it will be necessary to correct following tests for this effect.
filename <- paste0("plots/adiv_d21_feces_",TEST.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sdi, p.fpd, nrow = 1, labels = c("A)","B)","C)"), common.legend = TRUE,legend = "bottom")
(adiv_plot_title <- annotate_figure(adiv_plot, top = text_grob("Difference in alpha diversity between feed at day 0:", color = "red", face = "bold", size = 8)))
adiv_plot_title
...
## FEED AND TREAT (Day 21 only feces)
...
### Create output plot
filename <- paste0("plots/adiv_d21_",OUTER.VAR,"_",INNER.VAR,".png")
adiv_plot <- ggarrange(p.obs,p.sha, nrow = 1, labels = c("A)","B)"), common.legend = TRUE,legend = "bottom",align = "hv")
suppressMessages(ggsave(filename = filename, plot = adiv_plot, device = "png"))
...
ggplot(adat_long[!is.na(adat_long$Adiv_change) & adat_long$Day != 0,], aes(feed, Adiv_change, color = treatment)) + geom_boxplot() + facet_grid(Index~Day)

```
## DAY AND TREAT (NESTED)
...
#### Test project variable
### Observed richness
fit <- aov(as.formula(paste("samplePFOS_ug ~", OUTER.VAR,"*",INNER.VAR, sep = " ")), data = adat)
anova(fit)
TukeyHSD(fit)
...
stat.test <- adat %>%
  group_by(.data[[OUTER.VAR]]) %>%
  wilcox_test(as.formula(paste("samplePFOS_ug ~", INNER.VAR, sep = " "))) %>%
  adjust_pvalue(method = "BH") %>%
  add_significance("p.adj") %>% 
  add_xy_position(x = OUTER.VAR, dodge = 0.8) %>%
  p_format("p.adj", accuracy = 0.0001, trailing.zero = TRUE, new.col = TRUE)
...
# Perform pairwise comparisons
stat.test2 <- adat %>%
  wilcox_test(as.formula(paste("samplePFOS_ug ~", OUTER.VAR, sep = " "))) %>%
  adjust_pvalue(method = "BH") %>%
  add_significance("p.adj") %>% 
  add_xy_position(x = OUTER.VAR) %>%
  p_format("p.adj", accuracy = 0.0001, trailing.zero = TRUE, new.col = TRUE)
...
# Create plot
p <- ggboxplot(adat, x = OUTER.VAR, y = "samplePFOS_ug",
          color = INNER.VAR, palette = "jco",
          add = "jitter")
...
When testing contious variables I will recommend to first test for correlations between all relevant variables. If two or more variables are strongly correlated it is not necessary to test them all. Any identified batch effects should still be kept in mind when testing continuous variables. Also it can be relevant to include information about categorical variables in the analysis. The procedure is based on the suggestions from Statistical tools for high-throughput data analysis ([STHDA](http://www.sthda.com/english/wiki/correlation-test-between-two-variables-in-r))
...
I do not have sufficient continous variables to test for interactions, but I will perform the test for the calculated alpha diversity indeces.
...
# Select variables
num_var <- adat %>% select_if(is.numeric) %>% select(where(~n_distinct(.,na.rm = t) > 1)) %>% colnames()

num_var <- num_var[c(4,8:15)]

num_dat <- adat[,colnames(adat) %in% num_var]
...
num_var %>% 
  ...
  rowwise %>% 
  mutate(shapiro = list(
    adat[,value] %>% shapiro.test()
    ),
    shapiro_pval = shapiro$p.value
    )

# Plot all variables against each other
pairs(num_dat, pch = 19,  cex = 0.5,
      lower.panel=NULL)

# Run Pearson test
(corrmat <- cor(num_dat, method = "spearman", use = "complete.obs"))
...
# Visualise data
ggscatter(adat[adat$treatment != "CTRL",], x = VAR, y = INDEX, # basic plot
          facet.by = "feed", # This line can be commented out if alpha diversity is independent of batches
