...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = paste0("CEFTA_", Sys.Date(), '.html')) 
    })
---
...
First step is to load the output from the pipeline. The only part of this section that should be edited is the "grepl" commands in line 43 and 47 to ensure that negative controls and mock samples are properly identified

```{r Load_pipeline_data, eval = TRUE}
# Load analysis data
load("output/merged.phyloseq_object.RData")

# Create sample ID variable and use it as sample_names
sample_data(phy)$ID <- with(sample_data(phy), paste(plate_run,Sample, sep = ";"))
...
sample_data(phy)$is.neg <- grepl("WTRX|WTRP",sample_data(phy)$Sample,ignore.case = TRUE)
...
meta <- read.csv("CEFTA_metadata.csv", header = T)
...
> The included code is for both types of decontam and batches, delete lines as needed
> THIS SECTION MIGHT FAIL WHILE KNITTING (MEMORY INTENSIVE)
...
```{r rare_curve_plot, eval = TRUE, echo = TRUE}
# Load data
...
The last part of the initial analyses is to test differential abundance of individual ASVs. There are many different methods that can be used to calculate differential abundance, all with their own advantages and disadvantages. Which method to use depends on the data and therefore I will be using DAtest as described by [Russel et al. (2018)](https://doi.org/10.1101/241802).
