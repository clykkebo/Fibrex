...
## Body weight gain - Day 0-8 (n = 48) {.tabset .tabset-fade .tabset-pills}
...
## Body weight gain - Day 0-21  (n = 24) {.tabset .tabset-fade .tabset-pills}
This section will prepare to perform the data analysis for body weight gain
...
## Body weight gain - Day 8-21  (n = 24) {.tabset .tabset-fade .tabset-pills}
This section will prepare to perform the data analysis for body weight gain
...
## Body weight over time {.tabset .tabset-fade .tabset-pills}
...
## Cecum weight {.tabset .tabset-fade .tabset-pills}
...
## Liver weight {.tabset .tabset-fade .tabset-pills}
...
#### ug/mL {.tabset .tabset-fade .tabset-pills}
##### Prepare data
```{r , eval=TRUE, echo=TRUE, error=TRUE}
# Load data
load("R_objects/animal_data.Rdata")
...
#### ug/g {.tabset .tabset-fade .tabset-pills}
##### Prepare data
...
#### mg {.tabset .tabset-fade .tabset-pills}
...
### Blood serum data {.tabset .tabset-fade .tabset-pills}
...
### Liver data {.tabset .tabset-fade .tabset-pills}
...
### Brain data {.tabset .tabset-fade .tabset-pills}
...
### Feces data {.tabset .tabset-fade .tabset-pills}
...
### Cecum data {.tabset .tabset-fade .tabset-pills}
...
### Urine data {.tabset .tabset-fade .tabset-pills}
...
### Material test {.tabset .tabset-fade .tabset-pills}
...
## Concentration plots {.tabset .tabset-fade .tabset-pills}
...
# PH DATA {.tabset .tabset-fade .tabset-pills}
...
# TRANSIT TIME DATA {.tabset .tabset-fade .tabset-pills}
...
# CONSUMPTION DATA
## INTRODUCTION
Data on feed and water consumption was measured throughout the trial period (day 0 to 21) mainly with the scope of monitoring feed consumption between HF and LF groups. The two feed types, beside having high difference in fiber, were not isocaloric and had different contents of fats and protein with HF diet (Altromin) providing 3400 kcal/kg and LF diet (AIN-93G based D19090404) provided 4000 kcal/kg. A significantly larger body weight gain was observed in LF fed rats.
...
# Set names of variables
PREDICTOR <- "feedtreat"
OUTCOME <- "cal_cage_day_avr_g" # "feed_cage_day_avr_g"
SUBJECT <- "cage"
...
stat.in <- dat.clean %>%
  group_by(.data[[OUTER.VAR]]) %>%
  wilcox_test(as.formula(paste0(OUTCOME," ~ ",INNER.VAR)),
              paired = FALSE) %>%
  # adjust_pvalue(method = "bonferroni") %>%
  add_significance() %>%
  p_format("p", accuracy = 0.001, trailing.zero = TRUE, new.col = TRUE)
...
stat.out <- dat.clean %>%
  # group_by(.data[[FACETVAR]]) %>%
  wilcox_test(as.formula(paste0(OUTCOME," ~ ",OUTER.VAR)),
              paired = FALSE) %>%
  # adjust_pvalue(method = "bonferroni") %>%
  add_significance() %>%
  p_format("p", accuracy = 0.001, trailing.zero = TRUE, new.col = TRUE)
...
p <- ggboxplot(dat.clean, x = OUTER.VAR, y = OUTCOME,
  ...
  scale_fill_manual(values = params$COL1, name = "Group", labels = c("HF_CTRL" = "HF-CTRL","HF_PFOS" = "HF-PFOS","LF_CTRL" = "LF-CTRL","LF_PFOS" = "LF-PFOS")) +
  scale_color_manual(values = c("black","black","black","black")) +
  scale_y_continuous(name = "Feed consumed per cage per day (g)",limits = c(120,240),breaks = seq(120,240,20)) + #limits = c(30,60),breaks = seq(30,60,5)) +
  guides(color = "none") +
  theme(axis.title.x = element_blank())
...
# Output plot
ggsave(filename = paste0("plots/animal_data/consumption/",OUTCOME,"_plot.png"), p, device = "png", dpi = 300, units = "mm", width = 100, height = 100)
ggsave(filename = paste0("plots/animal_data/consumption/",OUTCOME,"_plot.pdf"), p, device = "pdf", dpi = 300, units = "mm", width = 100, height = 100)
...
p <- ggboxplot(dat.clean, x = OUTER.VAR, y = OUTCOME,
  ...
  scale_fill_manual(values = params$COL1, name = "Group", labels = c("HF_CTRL" = "HF-CTRL","HF_PFOS" = "HF-PFOS","LF_CTRL" = "LF-CTRL","LF_PFOS" = "LF-PFOS")) +
  scale_color_manual(values = c("black","black","black","black")) +
  scale_y_continuous(name = "Kcal intake per cage per day (kcal)",limits = c(120,240),breaks = seq(120,240,20)) + #limits = c(30,60),breaks = seq(30,60,5)) +
  guides(color = "none") +
  theme(axis.title.x = element_blank())

p
p <- p + stat_pvalue_manual(stat.in, label = "p.adj.format",tip.length = 0, hide.ns = FALSE)+
  stat_pvalue_manual(stat.out, label = "p.adj.format", tip.length = 0, hide.ns = FALSE)
