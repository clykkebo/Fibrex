...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = paste0("Fibrex_", Sys.Date(), "_ImportQC.html")) 
    })
params:
...
This document contains the commands necessary to perform initial import and QC of the output from the DF_GMH_PIPELINE. Here it has been fitted for data from Fibrex (project B-02-22). The project data loaded here contains 16S V2 amplicon sequencing from feces samples from 6 different days and cecum and ileum samples from two dissections originating from 48 rats. There are feces samples from all rats from day 0 and 8, while remaining days only contain from 24, as half the rats were dissected on day 8. Cecum and ileum samples were accordingly collected in at two dissection on day 8 and 21. Sampling on day 20 was run as a test of effect of carmine treatment for transit time.
...
- Weight data:
  + Body weight per sampling day (day *0*, *8*, *12*, *16*, *20* and *21*)
...
Division of groups are based on **Sample identifiers**:
  ...
  - "feed" given during the study: 
    + *LF* = low fibre (AIN-93G, D19090404 with non-lactococci casein, Research Diets)
