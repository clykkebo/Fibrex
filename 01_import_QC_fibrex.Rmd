...
This document contains the commands necessary to perform initial import and QC of the output from the DF_GMH_PIPELINE. Here it has been fitted for data from Fibrex (project B-02-22). The project data contains 16S V2 amplicon sequencing from feces samples from 6 different days and cecum and ileum samples from two dissections originating from 48 rats. There are feces samples from all rats from day 0 and 8, while remaining days only contain from 24, as half the rats were dissected on day 8. Cecum and ileum samples were accordingly collected in at two dissection on day 8 and 21. Sampling on day 20 was run as a test of effect of carmine treatment for transit time.

Metadata added contains results from:

- Carmine treatment (0 = no and 1 = yes) for different days.

- Body weight per sampling day (day 0, 8, 12, 16, 20 and 21), and cecum and liver weight per dissection day (day 8 and 21).
