...
# Rarefaction curves
Rcurve_data <- function(physeq, ntables=10, step=250,maxdepth = max(sample_sums(physeq)), methods=c("Observed","Chao1","ACE","Shannon","Evenness","Simpson"), seedstart=500, verbose=FALSE) {
  require("vegan")
...
# Calculate alpha diversity
Calculate_alpha_div <- function(physeq, ntables=100, depth = round(min(sample_sums(physeq))*0.9), methods=c("Observed","Chao1","ACE","Shannon","Evenness","Simpson"), seedstart=500, verbose=FALSE) {
  require("vegan")
...
First step is to load the output from the pipeline. The only part of this section that should be edited is the "grepl" commands to ensure that negative controls and mock samples are properly identified. For this ASV table the run column is called "Seq_run" and sample names are listed in "Sample". These are joined together to create the ID column used for fusion with metadata further down.
...
Next step is to load the metadata for the project. This step will have to be edited to fit the files and names used. Run and sample name columns corresponding to the ASV table are called "run" and "sample_name" in the metadata file.
...
meta$ID <- with(meta, paste(run,sample_name, sep = "_"))
...
Now that metadata and the ASV table both are loaded we fused the metadata information into the phyloseq element based on the ID and sampleID columns.
...
mtmp <- merge(tmp,meta,by.x="ID", by.y = "sampleID")
...
R.methods <- c("Observed","Chao1","ACE","Shannon")
...
plot.group <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = Seq_run)) + 
  geom_smooth(aes(group = Seq_run), method = "loess", formula = y ~ x, se = FALSE) + 
  facet_wrap("Index", scales = "free",nrow = 1) + 
  geom_vline(color = "red",xintercept = 13000) + 
  theme_pubclean() + scale_color_brewer(palette = "Paired")
