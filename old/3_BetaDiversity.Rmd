---
title: "Fibrex Microbiome Beta Diversity"
author: "masmo, caly"
date: "`r Sys.Date()`"
output:
...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = paste0("Fibrex_", Sys.Date(), "_BetaDiversity.html")) 
    })
params:
...
```{r calc_bray_subs, eval=FALSE, echo=TRUE}
params <- readRDS("R_objects/bdiv_params.RDS")
# load
load("R_objects/Phyloseq_rarefied.Rdata")
...
# Create subset
phy.sub <- subset_samples(phy.rare, material == "Feces" & day %in% c(0,8,21))
...
# Save distance objects
save(phy.sub, unif.sub.dist, unif.sub.nmds, unif.sub.pcoa, file = "R_objects/unif.sub.RData")
...
```{r calc_wUniF_subs, eval=FALSE, echo=TRUE}
params <- readRDS("R_objects/bdiv_params.RDS")
# load
load(params$input)
...
# Create subset
phy.sub <- subset_samples(phy, material == "Feces" & day %in% c(0,8,21))
...
# Save distance objects
save(phy.sub, wuf.sub.dist, wuf.sub.nmds, wuf.sub.pcoa, file = "R_objects/wUniF.sub.RData")
...
# Save distance objects
save(phy.sub, bray.sub.dist, bray.sub.nmds, bray.sub.pcoa, file = "R_objects/bray.sub.RData")
...
mdat$day <- as.factor(mdat$day)
(perm.test <- adonis2(unif.sub.dist ~ feed+day+treatment, data = mdat, permutations = 9999,na.action = na.omit))

# Use vegan to test how well metadata fits ordination
fit.out <- envfit(unif.sub.nmds, mdat[,c("feed","treatment","day")],na.rm=TRUE)
fit.out
```
...
unif.pcoa.tab <- plot_ordination(phy.sub, unif.sub.pcoa,axes = 1:5,justDF = TRUE)
unif.nmds.tab <- plot_ordination(phy.sub, unif.sub.nmds,axes = 1:5,justDF = TRUE)
wunif.pcoa.tab <- plot_ordination(phy.sub, wuf.sub.pcoa,axes = 1:5,justDF = TRUE)
wunif.nmds.tab <- plot_ordination(phy.sub, wuf.sub.nmds,axes = 1:5,justDF = TRUE)
bray.pcoa.tab <- plot_ordination(phy.sub, bray.sub.pcoa,axes = 1:5,justDF = TRUE)
bray.nmds.tab <- plot_ordination(phy.sub, bray.sub.nmds,axes = 1:5,justDF = TRUE)

# Reformat tables to create one common table
colnames(unif.nmds.tab)[1:5] <- c("Axis.1","Axis.2","Axis.3","Axis.4","Axis.5")
colnames(wunif.nmds.tab)[1:5] <- c("Axis.1","Axis.2","Axis.3","Axis.4","Axis.5")
colnames(bray.nmds.tab)[1:5] <- c("Axis.1","Axis.2","Axis.3","Axis.4","Axis.5")

unif.nmds.tab$ordination <- "nmds"
unif.pcoa.tab$ordination <- "pcoa"
wunif.nmds.tab$ordination <- "nmds"
wunif.pcoa.tab$ordination <- "pcoa"
bray.nmds.tab$ordination <- "nmds"
bray.pcoa.tab$ordination <- "pcoa"

unif.nmds.tab$metric <- "unif"
unif.pcoa.tab$metric <- "unif"
wunif.nmds.tab$metric <- "wuf"
wunif.pcoa.tab$metric <- "wuf"
bray.nmds.tab$metric <- "bray"
bray.pcoa.tab$metric <- "bray"


ord.tab <- rbind(unif.nmds.tab,unif.pcoa.tab,wunif.nmds.tab,wunif.pcoa.tab,bray.nmds.tab,bray.pcoa.tab)
ord.tab$day <- as.factor(ord.tab$day)
...
ggplot(axis.tab, aes_string(x = "ordination", y = "position", fill = "feed_treat")) +
  geom_boxplot() +
...
plot.unif <- ggplot(ord.tab[ord.tab$metric == "unif",], aes(x = Axis.1, y = Axis.2, color = feed_treat)) +
  geom_point() + 
  facet_grid(day~ordination, scales = "free") + 
  theme_pubr(legend = "bottom") + 
  labs(color="feed") +
  stat_ellipse() + ggsci::scale_color_jco() + ggtitle("Unweighted UniFrac")
plot.wuf <- ggplot(ord.tab[ord.tab$metric == "wuf",], aes(x = Axis.1, y = Axis.2, color = feed_treat)) +
  geom_point() + 
  facet_grid(day~ordination, scales = "free") + 
  theme_pubr(legend = "bottom") + 
  labs(color="feed") +
  stat_ellipse() + ggsci::scale_color_jco() + ggtitle("Weighted UniFrac")
plot.bray <- ggplot(ord.tab[ord.tab$metric == "bray",], aes(x = Axis.1, y = Axis.2, color = feed_treat)) +
  geom_point() + 
  facet_grid(day~ordination, scales = "free") + 
  theme_pubr(legend = "bottom") + 
  labs(color="feed") +
  stat_ellipse() + ggsci::scale_color_jco() + ggtitle("Bray-Curtis")
ggsave()
ggarrange(plot.unif, plot.wuf, plot.bray, nrow = 1)
```
